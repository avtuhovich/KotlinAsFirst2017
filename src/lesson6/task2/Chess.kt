...
data class Square(val column: Int, val row: Int) {
     ...
     */
    fun notation(): String {
        return if (column - 1 + 'a'.toInt() !in 'a'.toInt()..'h'.toInt() || this.row !in 1..8) ""
        else "${(column - 1 + 'a'.toInt()).toChar()}${this.row}"
    }
}
...
fun rookTrajectory(start: Square, end: Square): List<Square> {
    ...
    res.add(Square(start.column, start.row))
    if (start.column != end.column) res.add(Square(end.column, start.row))
    if (start.row > end.row || start.row < end.row) res.add(Square(end.column, end.row))
    return res
}
...
fun bishopMoveNumber(start: Square, end: Square): Int =
        if (start.inside() && end.inside()) {
            when {
                ...
                (abs(start.column - end.column) + abs(start.row - end.row)) % 2 == 0 -> 2
                else -> -1
            }
        } else
            throw IllegalArgumentException("IllegalArgumentException")
...
fun bishopTrajectory(start: Square, end: Square): List<Square> {
    ...
    var a = (end.row + start.row + end.column - start.column) / 2
    var b = a - start.row + start.column
    if (a !in 1..8 || b !in 1..8) {
        a = (end.row + start.row - end.column + start.column) / (2)
        b = a - end.row + end.column
    }
    return listOf(start, Square(b, a), end)
}
